...
DELAY1			= 300		#Задержка запуска после синхроимпульса в микросекундах По умолчанию
DELAY2			= 500		#Задержка запуска после синхроимпульса в микросекундах По умолчанию
...
# расчет CRC32 короткой строки, длина строки дожна быть кратной 4
...
def change_delay(arr):
    ...
    for i in range(40, df.shape[0] - 4):
        if (df.iloc[i - 2]['2'] < 0 and df.iloc[i - 1]['2'] < 0 and df.iloc[i]['2'] < 0 and df.iloc[i + 1]['2'] < 0 and
                df.iloc[i + 2]['2'] < 0):  # проверка по второй производной
            for j in range(-2, 2):
                df._set_value(i, '3', 570)
        # проверка по знаку первой производной
        if (df.iloc[i - 4]['1'] > 0 and df.iloc[i - 3]['1'] > 0 and df.iloc[i - 2]['1'] > 0 and df.iloc[i - 1][
            '1'] > 0 and df.iloc[i + 1]['1'] < 0 and df.iloc[i + 2]['1'] < 0 and
                df.iloc[i + 3]['1'] < 0 and df.iloc[i + 4]['1'] < 0):  # df.iloc[i]['1'] == 0 and
    ...
    picks = df[
        df['3'] > (df['3'].max() * 0.95)].index.tolist()  # получение значений с максимальным совпадением признаков
    print(picks)
    true_picks = []
    if (len(picks) > 0 and len(picks) < 3):
...
col = iter (num_list)
tk.Button(root,text="Start",command=lambda x = 1: SendControl(x)).grid(row=1, column = next(col), sticky='nesw') # Create a tkinter button
tk.Button(root,text="Stop",command=lambda x = 0: SendControl(x)).grid(row=1, column = next(col), sticky='nesw') # Create a tkinter button
tk.Button(root,text="Fire",command=lambda x = 0xFE: SendControl(x)).grid(row=1, column = next(col), sticky='nesw') # Create a tkinter button
tk.Button(root,text="Exit",command=Exit).grid(row=1, column = next(col), sticky='nesw') # Create a tkinter button


#Delays input
del1=tk.StringVar(value=DELAY1)
del2=tk.StringVar(value=DELAY2)

tk.Entry(root, width=8, textvariable=del1).grid(row=1, column=4, sticky='s')
tk.Entry(root, width=8, textvariable=del2).grid(row=1, column=6, sticky='s')
tk.Label(root, text='Delay1').grid(row=1,column=5,sticky='s')
tk.Label(root, text='Delay2').grid(row=1,column=7,sticky='s')

tk.Button(root,text='Apply Changes', command=Apply_changes).grid(row=1,column=10, sticky='nesw')
...
tk.Checkbutton(root,variable=var1,onvalue=1, offvalue=0).grid(row=1,column=8,sticky='nesw')
tk.Checkbutton(root,variable=var2,onvalue=1, offvalue=0).grid(row=1,column=9,sticky='nesw')
tk.Label(root, text='Channel1').grid(row=0,column=8,sticky='s')
tk.Label(root, text='Channel2').grid(row=0,column=9,sticky='s')
