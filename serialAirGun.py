...
def change_delay(arr):
    ...
    def find_pick(FB):
        ...
        c = np.zeros(len(FB))
        d = np.zeros(len(FB))
        signal_array = np.column_stack((FB, b, c, d))
        filt_low = []
        ...
        for i in range(len(signal_array)):
            filt.append(1)
        for i in range(100, len(signal_array) - 100):  # ширина фильтра
            filt[i] = 0
        sigfft = sp.fft.fft(signal_array[:, 0])
        for i in range(len(signal_array)):
            sigfft[i] *= filt[i]
        sigres = sp.fft.ifft(sigfft).real

        for i in range(len(signal_array)):
            signal_array[i, 0] = abs(sigres[i])
        signal_array = signal_array[0: min_time_ms10 + window_width10 + 10, :]
        for i in range(1, len(signal_array) - 1):
            signal_array[i, 3] = 0
            signal_array[i, 1] = signal_array[i + 1, 0] - signal_array[i, 0]  # расчет первая производная
            signal_array[i, 2] = signal_array[i, 1] - signal_array[i - 1, 1]  # расчет вторая производная

        signal_array[0, 3] = 0
        signal_array[-1, 3] = 0

        for i in range(40, len(signal_array) - 4):
            if signal_array[i - 2, 2] < 0 \
                    and signal_array[i - 1, 2] < 0 \
                    and signal_array[i, 2] < 0 \
                    and signal_array[i + 1, 2] < 0 \
                    and signal_array[i + 2, 2] < 0:  # проверка по второй производной
                for j in range(-2, 2):
                    signal_array[i, 3] = 570
            # проверка по знаку первой производной
            if signal_array[i - 4, 1] > 0 \
                    and signal_array[i - 3, 1] > 0 \
                    and signal_array[i - 2, 1] > 0 \
                    and signal_array[i - 1, 1] > 0 \
                    and signal_array[i + 1, 1] < 0 \
                    and signal_array[i + 2, 1] < 0 \
                    and signal_array[i + 3, 1] < 0 \
                    and signal_array[i + 4, 1] < 0:
                signal_array[i, 3] = signal_array[i, 3] + 530

        max_2 = signal_array[:, 1].max()
        for i in range(10, len(signal_array) - 10):  # проверка по наличию максимального значения 1-й производной
            up = False
            down = False
            condition_5 = False
            for j in range(-10, 0):
                if signal_array[i + j, 1] > max_2 * 0.9:
                    up = True
            for j in range(10):
                if signal_array[i + j, 1] < -max_2 * 0.9:
                    down = True
            if up and down:
                signal_array[i, 3] += 510
            for j in range(-10 + i, i):
                if signal_array[j, 0] > (signal_array[i, 0] * 0.7):
                    condition_5 = True
            if not condition_5:
                signal_array[i, 3] += 1000

        for i in range(len(signal_array) - 10, len(signal_array)):  # избавляемся от краевых эффектов
            signal_array[i, 3] = 0
        for i in range(0, min_time_ms10):
            signal_array[i, 3] = 0

        signal_array[maxvalueid, 3] += 500
        max3 = signal_array[:, 3].max() * 0.95
        picks = []
        for i in range(5, len(signal_array)):
            if signal_array[i, 3] > max3:
                picks.append(i)
